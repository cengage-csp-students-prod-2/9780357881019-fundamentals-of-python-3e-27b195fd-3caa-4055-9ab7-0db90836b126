...
def splitspace_v(image:images.Image,x1:int,y1:int,x2:int,y2:int):
    ...
    return (x1,y1,x1+dx,y2) if m==2 else (x1+dx,y1,x2,y2)
def splitspace_h(image:images.Image,x1:int,y1:int,x2:int,y2:int):
    if x2-x1<=3 or y2-y1<=3: return
    m=randint(1,2)
    dy=(y2-y1)//3*m
    draw_rectangle(image,x1,y1,x2,y1+dy,random_color())
    return (x1,y1,x2,y1+dy) if m==2 else(x1,y1+dy,x2,y2)

def splitspace(image:images.Image,coords,l:int,orientation:str='v'):
    x1,y1,x2,y2=coords
    if l==0:return
    if orientation=='h':
        splitspace(image,splitspace_v(image,x1,y1,x2,y2),l-1,'v')
    else:
        splitspace(image,splitspace_h(image,x1,y1,x2,y2),l-1,'h')
